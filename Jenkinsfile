...
pipeline {
    ...
    stages {
        ...
        stage('Unit test') {
            ...
            post {
                always {
                    archiveArtifacts artifacts: 'result.xml', fingerprint: true, allowEmptyArchive: true
                    junit 'result.xml'
                }
            }
        }
            ...
            steps {
                withCredentials([usernamePassword(
                    ...
                    sshagent(credentials:['cluster-credentials']) {
                        sh"""
                        ssh -o StrictHostKeyChecking=no redhat@172.31.43.117 docker rm -f ${JOB_BASE_NAME} || true
                        ssh -o StrictHostKeyChecking=no redhat@172.31.43.117 docker run -d --name ${JOB_BASE_NAME} -p 8080:9046 --pull always ${username}/${JOB_BASE_NAME}
                        """
                    }
                }
            }
